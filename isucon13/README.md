...
* 進捗は `/var/log/cloud-init-output.log` で確認できます
  ...
  ```sh
  multipass exec isucon13 -- tail -f /var/log/cloud-init-output.log
  ```

* ログインとIPアドレスの確認

  ```sh
  multipass shell isucon13
  multipass info isucon13
  ```

* 環境の停止・再開・削除

  ```sh
  multipass stop isucon13
  multipass start isucon13
  multipass delete --purge isucon13
  ```
